...
void ADungeonEscapeCharacter::Interact()
{
	...
	FVector End = Start + (GetFirstPersonCameraComponent()->GetForwardVector() * MaxInteractionDistance);
	FCollisionShape InteractionSphere = FCollisionShape::MakeSphere(InteractionSphereRadius);
	DrawDebugSphere(GetWorld(), End, InteractionSphereRadius, 12, FColor::Green, false, 5.0f);
	...
	UE_LOG(LogDungeonEscape, Log, TEXT("Interacted"));
	if (bHasHit)
	{
		...
		if (HitActor)
		{
			...
			{
				UE_LOG(LogDungeonEscape, Log, TEXT("Collected item: %s"), *HitActor->GetName());
			...
			{
				UE_LOG(LogDungeonEscape, Log, TEXT("Interacted with lock: %s"), *HitActor->GetName());
				ALock* Lock = Cast<ALock>(HitActor);
				if (Lock)
				{
					UE_LOG(LogDungeonEscape, Log, TEXT("Lock key item name: %s"), *Lock->KeyItemName);
					if (ItemList.Contains(Lock->KeyItemName) && !Lock->GetbIsKeyPlaced())
					{
							ItemList.RemoveSingle(Lock->KeyItemName);
							Lock->SetbIsKeyPlaced(true);
					}
					else if (!ItemList.Contains(Lock->KeyItemName) && Lock->GetbIsKeyPlaced())
					{
						ItemList.Add(Lock->KeyItemName);
						Lock->SetbIsKeyPlaced(false);
					}
				}
			}
			else
	...
	{
		UE_LOG(LogDungeonEscape, Log, TEXT("There's no interactable object hit"));
	}
